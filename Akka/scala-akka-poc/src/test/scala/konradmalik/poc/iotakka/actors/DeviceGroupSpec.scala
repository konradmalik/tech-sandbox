...
class DeviceGroupSpec extends TestKit(ActorSystem("deviceGroupSpec")) with FlatSpecLike with Matchers {

  "GroupActor" should "be able to register a device actor" in {
    val probe = TestProbe()
  ...
  it should "ignore request for wrong groupId" in {
    val probe = TestProbe()
    val groupActor: ActorRef = system.actorOf(DeviceGroup.props("group"))
